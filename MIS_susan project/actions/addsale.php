<?php 
    ...
    if (isset($_POST['tcode']) && isset($_POST['amt_sold'])) {
        ...
        // Retrieve the current available amount of tea from inventory
        $query_inventory = "SELECT itemamt FROM inventory WHERE itemname = 'tea'";
        ...
        // Check if there is enough tea to fulfill the sale
        if ($availableAmt >= $amt_sold) {
            // Update the inventory for 'tea'
            $new_amt = $availableAmt - $amt_sold;
            $update_inventory = "UPDATE inventory SET itemamt = $new_amt WHERE itemname = 'tea'";
            ...
            // Inform the user that there's not enough tea in the inventory
            $restxt = "Not enough tea in inventory.";
        }
    } else {
