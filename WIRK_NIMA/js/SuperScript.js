...
// this one expands the toggleshow functionality

function toggleShowAll(us) {
	//requires data-shown parameter to be attached to HTML object me
	var item = document.querySelectorAll(us),def = 0;

	item.forEach((tempitem) => {
		if(tempitem.dataset.shown == null){
			def = tempitem.style.display == "none" ? 0 : 1;
		} else {
			def = Number(tempitem.dataset.shown);
		}

		if(def == 0){
			tempitem.style.display = 'block';
			tempitem.dataset.shown = 1;
		} else {
			tempitem.style.display = 'none';
			tempitem.dataset.shown = 0;
		}
	});
}