...
if (isset($_GET['vname'])) {
    ...
    $sql = "DELETE FROM vendors WHERE vname = '$pname' AND eventid = $curevent";

    // Execute the SQL statement
    if ($conn->query($sql) === TRUE) {
        echo "vendor fired successfully.";
    } else {
        echo "Error: Could not execute query: " . $conn->error;
    }
} else {
    echo "Error: vendor name not provided.";
}
echo "redirecting";
...
setTimeout(function() {
    window.location.href = 'view_vendors.php'; // Adjust the file name if necessary
}, 2000);
</script>
