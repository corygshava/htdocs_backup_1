<?php 
    ...
    if (isset($_POST['f_id']) && isset($_POST['in_amt'])) {
        ...
        $farmer_query = "SELECT intakes, seedsbrought FROM farmers WHERE id = '$f_id'";
        $result_farmer = $conn->query($farmer_query);

        if ($result_farmer && $result_farmer->num_rows > 0) {
            // Get price per kilo
            ...
            if ($available_balance >= ($in_amt * $theprice)) {
                ...
                $seedsamt = $farmer_data['seedsbrought'] + $in_amt;

                // Update farmers table
                $update_farmers = "UPDATE farmers SET intakes = '$fintakes', seedsbrought = '$seedsamt' WHERE id = '$f_id'";
                ...
                // Update inventory
                $update_inventory = "UPDATE inventory SET itemamt = itemamt + '$in_amt' WHERE itemname = 'coffee_seeds'";
